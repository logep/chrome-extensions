...
CD.network.onRequestFinished.addListener((...args)=>{
    ...
    if(response && response.status){
        ...
        let tp="<tr><td></td>" +
            "<td class='url'><div><a href='"+request.url+"' >'"+request.url+"'</a>" +
        "</div></td><td ></td> <td ></td></tr>";
        document.querySelector(".list").appendChild(tp);
        const addEvent = document.querySelector('.url')
        addEvent.addEventListener('click',onUrlClick)
    }
...
function onUrlClick(event)
{
    ...
    if (!("openResource" in chrome.devtools.panels))
        return;

    chrome.devtools.panels.openResource(event.target.href);
    event.preventDefault();
}

// elementsPanel.onShown.addListener((...args)=>{
//     chrome.storage.local.get("ajaxInterceptor_rules",function (dt) {
//         document.querySelector('#devPanel').value="55423423"+Math.random()
//         log("elementsPanel")
//         log(event)
...
{
    const container = document.getElementById("devPanel");
    container
    ...
    {
        document.querySelector('#devPanel').value="4"+Math.random()
        log(message)
        log(4444444334343434)
    });
    window.addEventListener("message", (event) =>
    {
        document.querySelector('#devPanel').value="66664"+Math.random()
        log(event)
        log(77777777777778787878)
    });
}, false);
// document.querySelector('#devPanel').value="4"+Math.random()
...
//     log(namespace)
//     document.querySelector('#devPanel').value="55423423"+Math.random()
//     log('backgroud namespace')
//     for (key in changes) {
//         var storageChange = changes[key];
//         // log(key, namespace, storageChange.oldValue, storageChange.newValue);
//     }
// });
// extensionPanel.onShown.addListener((...args)=>{
//     document.querySelector('#devPanel').value="55423423"+Math.random()
//     chrome.storage.local.get("ajaxInterceptor_rules",function (dt) {
//         log("extensionPanel")
//         document.querySelector('#devPanel').value="55423423"+Math.random()
